...
### Installation
A data and document hub for Poolula LLC that combines property operations, financial tracking, and AI-assisted Q&A over structured transactions and business documents.

<p align="center">
  <a href="#"><img src="https://img.shields.io/badge/CI-passing-brightgreen" alt="CI Status"></a>
  <a href="#"><img src="https://img.shields.io/badge/Python-3.13+-blue" alt="Python Version"></a>
  <a href="#"><img src="https://img.shields.io/badge/Framework-FastAPI-009688" alt="FastAPI"></a>
  <a href="#"><img src="https://img.shields.io/badge/License-Private-red" alt="License"></a>
</p>
...
## 🚀 Quick Start
Get the API and chatbot running locally.
...
### Usage
Open [http://localhost:8082/docs](http://localhost:8082/docs) for interactive Swagger UI.
...
```
## 🧭 Project Overview
Poolula Platform centralizes property data, transactions, and key documents, then layers a FastAPI service and RAG-enabled chatbot to answer compliance and financial questions with source citations.
...
```mermaid
flowchart LR
  Sources[Data Sources\n(Airbnb CSV, Bank Statements, Manual)] -->|imports| DB[(SQLModel DB)]
  Docs[Business Documents\n(PDF/DOCX)] -->|ingest| Vectors[Vector Store]
  DB --> API[FastAPI Service]
  Vectors --> API
  API --> Chat[Chatbot & Evaluation]
  Chat --> User[CLI / Frontend]
```
...
uv run python scripts/import_airbnb_transactions.py \
    ...
    --property-id <uuid>
## ✨ Features
- Property, transaction, document, and obligation management with provenance tracking.
- RESTful CRUD API (`/api/v1`) plus health and docs endpoints.
- RAG-based chatbot for compliance and finance Q&A with citation logging.
- Import scripts for Airbnb CSVs, bank statements, and document ingestion.
- Evaluation harness for measuring AI response quality against golden answers.
- Soft-delete semantics and audit logging across API and chatbot activity.
...
│  ├─ chatbot/              # Chat tools and pipelines
│  └─ evaluator/            # Evaluation harness
├─ core/                    # Database models, enums, logging
├─ scripts/                 # CLIs (seed, import, backup)
├─ alembic/                 # Database migrations
├─ docs/                    # MkDocs site, architecture & workflows
├─ tests/                   # Pytest suite
...
## ⚙️ How It Works
Core components collaborate to serve both API requests and chatbot conversations.
...
sequenceDiagram
  ...
  User->>API: CRUD request or chat query
  API->>DB: Validate & fetch structured data
  API->>Vec: (Chat) Retrieve similar docs
  API->>Chat: Compose prompt with data + docs
  Chat-->>API: Answer with citations
  API-->>User: JSON response (data or answer)
```
...
- **Environment variables**: Use a `.env` file or shell exports for secrets (e.g., `ANTHROPIC_API_KEY`, database URL, optional OpenAI keys).
- **Database**: Defaults to SQLite (see `core/database/connection.py`); configure via environment variable for other backends.
- **Vector store**: ChromaDB for embeddings; ensure dependencies from the `rag` group are installed when enabling RAG flows.
- **Settings management**: `pydantic-settings` is used for typed configuration.
...
curl -X POST http://localhost:8082/api/v1/transactions \
  ...
  -d '{"property_id": 1, "transaction_date": "2025-01-01", "amount": 1200, "category": "RENTAL_INCOME", "transaction_type": "REVENUE"}'
```
...
curl -X POST http://localhost:8082/api/query \
  -H "Content-Type: application/json" \
  -d '{"query": "What was rental income in August 2025?"}'
```
...
![Screenshot: Main UI](docs/images/main-ui.png)

## 🧑‍💻 Development Guide
- **Run tests**: `uv run pytest`
- **Coverage HTML**: `uv run pytest --cov=core --cov=apps --cov-report=html`
...
|--------------|---------------------------|------------------------------------------|
| API          | FastAPI + Uvicorn         | HTTP REST + interactive docs             |
| Data         | SQLModel + Alembic        | ORM-style models and migrations          |
| AI / RAG     | Anthropic API + ChromaDB  | LLM responses with vector search         |
| Docs Site    | MkDocs + Material theme   | Developer and architecture docs          |
| Tooling      | uv, Ruff, Pytest, Mypy    | Env management, linting, testing, typing |

## 🗺️ Roadmap
- Fix ChromaDB document search bug and re-ingest LLC documents.
- Port frontend with persona-based views and dashboards.
- Expand golden evaluation set (15 → 40 questions) and improve scoring.
- Build evaluation reporting dashboard and aim for ≥90% accuracy.

## 🤝 Contributing
Contributions are welcome via issues and pull requests. Please follow existing coding standards (Ruff, Pytest, Mypy) and keep architectural decisions documented in `docs/architecture/`.

## 📜 License
Currently proprietary for Poolula LLC internal use. If you need to use or adapt this code, please contact the maintainers.